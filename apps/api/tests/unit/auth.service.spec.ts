...
describe('AuthService.register — US-01', () => {
  ...
  describe('Criterio 3: email único', () => {
    it('lanza ConflictError si el email ya existe', async (context) => {
      ...
      await link(context, 'custom', 'https://github.com/FacundoAcostaG/taskflow/blob/main/apps/api/src/services/auth.service.ts', 'AuthService.register')
      await description(context, 'Verifica que el registro rechaza un email duplicado y preserva la unicidad del usuario.')

      mockDb.user.findUnique.mockResolvedValue(mockUser)

      await step(context, 'Intentar registrar un usuario con email ya existente', async () => {
        await expect(
          authService.register(validRegisterInput)
        ).rejects.toThrow(ConflictError)
      })

      await step(context, 'Validar el mensaje funcional del conflicto', async () => {
        await expect(
          authService.register(validRegisterInput)
...
describe('AuthService.login — US-02', () => {
  ...
  describe('Criterio 1: login exitoso retorna JWT', () => {
    it('retorna token para credenciales válidas', async (context) => {
      ...
      await link(context, 'custom', 'https://github.com/FacundoAcostaG/taskflow/blob/main/apps/api/src/services/auth.service.ts', 'AuthService.login')
      await description(context, 'Confirma que un usuario válido puede autenticarse y recibir un JWT para continuar en la aplicación.')
      ...
      await step(context, 'Autenticar al usuario con credenciales válidas', async () => {
        result = await authService.login({ email: 'ana@test.com', password: 'Password1' })
      })

      await step(context, 'Verificar que la respuesta incluye token y datos del usuario', async () => {
        expect(result.token).toBeDefined()
        expect(result.user.email).toBe('ana@test.com')
      })
    })
  ...
  describe('Criterio 3: bloqueo por intentos fallidos', () => {
    it('bloquea la cuenta después de 5 intentos fallidos — BUG-05', async (context) => {
      ...
      await story(context, 'US-02')
      await severity(context, 'critical')
      await link(context, 'custom', 'https://github.com/FacundoAcostaG/taskflow/blob/main/apps/api/src/services/auth.service.ts', 'BUG-05 auth lockout')
      await description(context, 'Documenta el comportamiento del bloqueo por intentos fallidos y expone el bug off-by-one asociado al umbral.')
      ...
      await step(context, 'Ejecutar un nuevo login fallido en el límite de intentos', async () => {
        await expect(
          authService.login({ email: 'ana@test.com', password: 'WrongPass1' })
        ).rejects.toThrow(UnauthorizedError)
      })
      ...
      await step(context, 'Comprobar que el servicio intenta establecer el bloqueo', async () => {

...
describe('TaskService.validateTitle', () => {
  ...
  it('debe lanzar error si el titulo tiene menos de 3 caracteres', () => {
    expect(() =>
      svc['validateTitle']('a'.repeat(MIN_CHAR_TASK_NAME - 1))
    ).toThrow()
  })

  it('debe lanzar error si el titulo tiene mas de 100 caracteres', () => {
    expect(() =>
      svc['validateTitle']('a'.repeat(MAX_CHAR_TASK_NAME + 1))
    ).toThrow()
  })
  ...
  it('debe aceptar un titulo con exactamente 100 caracteres (valor limite superior)', () => {
    expect(() =>
      svc['validateTitle']('a'.repeat(MAX_CHAR_TASK_NAME))
    ).not.toThrow()
  })
})
...
describe('TaskService.validateStatusTransition', () => {
  ...
  it('debe permitir la transicion TODO -> IN_PROGRESS', () => {
    expect(() =>
      svc['validateStatusTransition'](Status.TODO, Status.IN_PROGRESS)
    ).not.toThrow()
  })

  it('debe permitir la transicion IN_PROGRESS -> DONE', () => {
    expect(() =>
      svc['validateStatusTransition'](Status.IN_PROGRESS, Status.DONE)
    ).not.toThrow()
  })

  it('debe rechazar la transicion TODO -> DONE (salto de estado)', () => {
    expect(() =>
      svc['validateStatusTransition'](Status.TODO, Status.DONE)
    ).toThrow()
  })

  it('debe permitir la transicion IN_PROGRESS -> TODO (reapertura controlada)', () => {
    expect(() =>
      svc['validateStatusTransition'](Status.IN_PROGRESS, Status.TODO)
    ).not.toThrow()
  })

  it('debe rechazar cualquier transicion desde DONE (estado final)', () => {
    expect(() =>
      svc['validateStatusTransition'](Status.DONE, Status.TODO)
    ).toThrow()
    expect(() =>
      svc['validateStatusTransition'](Status.DONE, Status.IN_PROGRESS)
    ).toThrow()
    expect(() =>
      svc['validateStatusTransition'](Status.DONE, Status.DONE)
    ).toThrow()
  })

  it('debe rechazar la transicion al mismo estado', () => {
    expect(() =>
      svc['validateStatusTransition'](Status.TODO, Status.TODO)
    ).toThrow()
    expect(() =>
      svc['validateStatusTransition'](Status.IN_PROGRESS, Status.IN_PROGRESS)
    ).toThrow()
    expect(() =>
      svc['validateStatusTransition'](Status.DONE, Status.DONE)
    ).toThrow()
...
describe('Ejercicios adicionales (opcional)', () => {
  ...
  it.todo(
    'validateTitle: titulo con espacios al inicio y al final - se recorta?'
  )
  it.todo(
    'validateStatusTransition: DONE -> DONE - mismo error que otras transiciones desde DONE?'
  )
})
